(()=>{
  ...
  const PLAN_RULES={
    free:{label:'Free',searches:1,credits:10,depth:10,csv:false},
    starter:{label:'Starter',searches:100,credits:50,depth:25,csv:true},
    growth:{label:'Growth',searches:300,credits:250,depth:50,csv:true},
  ...
  function parseGvizTable(payload){
    ...
    return (payload.table?.rows||[]).map((row,i)=>{const o={_row_index:i+2};(row.c||[]).forEach((cell,idx)=>o[cols[idx]]=cell?clean(cell.f||cell.v||''):'');return o;});
  }
  function fetchSheet(sheet){
    return new Promise(resolve=>{
      const cb='rfEffectiveAdmin_'+sheet+'_'+Date.now()+'_'+Math.floor(Math.random()*99999);
      const script=document.createElement('script');let done=false;
      const timer=setTimeout(()=>finish([]),12000);
      function finish(rows){if(done)return;done=true;clearTimeout(timer);try{script.remove()}catch{}try{delete window[cb]}catch{}resolve(rows||[]);}
      window[cb]=p=>{try{finish(parseGvizTable(p))}catch{finish([])}};
      ...
      script.async=true;script.onerror=()=>finish([]);document.body.appendChild(script);
    });
  }

  function normalizePlan(v){const raw=low(v).replace(/\s+/g,'_').replace(/-/g,'_');if(/admin|unlimited/.test(raw))return'admin';if(raw==='growth'||raw==='pro')return'growth';if(raw==='starter'||raw==='basic')return'starter';return'free';}
  ...
  function tie(r){if(isOverride(r))return 3;if(hasPaidSource(r))return 2;if(rowBilling(r)==='active'&&rowPlan(r)!=='free')return 1;return 0;}
  ...
  function limitFromRow(r,type,plan){
    const rule=PLAN_RULES[plan]||PLAN_RULES.free;
    const raw=type==='search'?first(r.effective_search_limit,r.monthly_search_limit,r.search_batches_limit,r.base_search_limit):first(r.effective_qualified_lead_limit,r.monthly_qualified_lead_credit_limit,r.qualified_lead_credits_limit,r.base_qualified_lead_limit);
    const val=raw?number(raw):0;
    if(raw&&val>0)return val;
    if(raw&&val===Infinity)return Infinity;
  ...
  function idUser(r){return first(rowUserId(r),r.user_id,r.owner_user_id,r.userId,rowEmail(r),r.email,r.user_email);}
  ...
  function matchUserRow(row,user){
    const em=rowEmail(row), uid=rowUserId(row), userEmail=user.email_key, userId=user.user_id;
    return (em&&userEmail&&em===userEmail)||(uid&&userId&&uid===userId)||(!em&&row.user_id&&clean(row.user_id)===clean(userId));
  }

  function buildModel(usersRows,searchRows,leadRows,feedbackRows){
    const groups={};
    usersRows.forEach(r=>{const k=canonicalKey(r);if(!groups[k])groups[k]=[];groups[k].push(r);});
    const output=Object.entries(groups).map(([key,rows])=>{
      const best=choose(rows)||rows[0]||{};
      const effectiveBilling=rowBilling(best);
      let effectivePlan=rowPlan(best);
      if(low(rowEmail(best))===ADMIN||effectiveBilling==='admin_unlimited')effectivePlan='admin';
      if(effectiveBilling!=='active'&&effectiveBilling!=='admin_unlimited'&&effectivePlan!=='admin')effectivePlan='free';
      const user={
        key,
        user_id:first(rowUserId(best),best.user_id,best.userId,key),
        email_key:rowEmail(best),
        email:rowEmail(best)||first(best.email,best.user_email,'No email recorded'),
        plan:effectivePlan,
        billing_status:effectiveBilling,
        source:first(best.source,''),
        priority:priority(best),
        source_row_index:best._row_index,
        search_limit:limitFromRow(best,'search',effectivePlan),
        credit_limit:limitFromRow(best,'credit',effectivePlan),
        review:0,searches:0,credits:0,leads:0,feedback:0,last_active_at:first(lastActivity(best),'')
      };
      return user;
    });
    const byKey=output;
    function findUser(row){return byKey.find(u=>matchUserRow(row,u));}
    const seenSearches=new Set();
    searchRows.forEach(r=>{const u=findUser(r);if(!u)return;const sid=idSearch(r)||('search_'+Math.random());const key=u.key+'|'+sid;if(!seenSearches.has(key)){seenSearches.add(key);u.searches++;}const d=lastActivity(r);if(d&&(!u.last_active_at||new Date(d)>new Date(u.last_active_at)))u.last_active_at=d;});
  ...
  async function load(){
    if(!isAdmin())return;
    status('Refreshing effective billing data…','warn');
    const [usersRows,searchRows,leadRows,feedbackRows]=await Promise.all([fetchSheet('users'),fetchSheet('searches'),fetchSheet('final_leads'),fetchSheet('lead_feedback')]);
    model.users=buildModel(usersRows,searchRows,leadRows,feedbackRows);
    model.loaded=true;
    window.rankforgeAdminEffectiveBilling=model;
    renderForTab();
    status('Effective billing data synced','ok');
  }

  function bind(){
    ['#userSearch','#userPlanFilter','#userBillingFilter'].forEach(sel=>$(sel)?.addEventListener(sel==='#userSearch'?'input':'change',()=>{if(activeTab()==='users')renderUsers();}));
    $$('.rf-admin-tabs a').forEach(a=>a.addEventListener('click',()=>setTimeout(renderForTab,250)));
    $('#refreshAdminData')?.addEventListener('click',()=>setTimeout(load,700));
    const obs=new MutationObserver(()=>{if(model.loaded)setTimeout(renderForTab,120);});
    const usersBody=$('#usersTable tbody'),billingBody=$('#billingTable tbody');
    if(usersBody)obs.observe(usersBody,{childList:true});
    if(billingBody)obs.observe(billingBody,{childList:true});
  }
