(function(){
  ...
  function profileMatchesSession(p,s){var em=low(s.email||s.userEmail);var uid=clean(s.userId||s.id);return !!p&&((em&&low(p.email)===em)||(uid&&clean(p.user_id)===uid));}
  function effective(){var s=session(),p=profile(),email=low((s&&s.email)||(s&&s.userEmail)||(p&&p.email));if(!p||!p.resolved_at||!profileMatchesSession(p,s)){return{ready:false,plan:'loading',name:'Plan loading',billing_status:'loading',searchLimit:null,creditLimit:null,maxBatch:null,csv:false,isAdmin:false};}var plan=normPlan(p.plan);var billing=normBilling(p.billing_status);if(email===ADMIN_EMAIL){plan='admin_unlimited';billing='admin_unlimited';}else if(plan==='admin_unlimited'){plan='free';billing='free';}
  ...
  function start(){render();if(window.rankforgeResolveEffectiveUserProfile){window.rankforgeResolveEffectiveUserProfile({force:true,reason:'app_plan_stabilizer'}).then(render).catch(render);}setTimeout(render,900);setTimeout(render,2500);}
