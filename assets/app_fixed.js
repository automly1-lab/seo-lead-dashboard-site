...
function leadStatus(lead) {
  return normalizeStatus(lead.status || lead.qualification_status || lead.qualificationStatus);
}

function listLeadCounts(listId) {
  const id = normalizeKey(listId);
  const leads = (runtimeData.leads || []).filter((lead) => normalizeKey(lead.listId) === id);
  return {
    qualified: leads.filter((lead) => leadStatus(lead) === "qualified").length,
    reviewNeeded: leads.filter((lead) => leadStatus(lead) === "review_needed" || leadStatus(lead) === "review needed").length,
    rejected: leads.filter((lead) => leadStatus(lead) === "rejected").length,
    total: leads.length,
  };
}
...
function renderDashboardPage() {
  ...
  const qualified = runtimeData.leads.filter((item) => leadStatus(item) === "qualified").length;
  const reviewNeeded = runtimeData.leads.filter((item) => leadStatus(item) === "review_needed" || leadStatus(item) === "review needed").length;
  ...
  if (!selected) return;
  const selectedCounts = listLeadCounts(selected.id);
  const selectedQualified = selectedCounts.qualified || numberValue(selected.qualified);
  const selectedReviewNeeded = selectedCounts.reviewNeeded || numberValue(selected.reviewNeeded);
  const selectedRejected = selectedCounts.rejected || numberValue(selected.rejected);
  setText("workspaceSelectedListHealth", selectedQualified > 0 ? "List has qualified leads" : (selectedReviewNeeded > 0 ? "List has review-needed leads" : "List needs review"));
  setText("workspaceSelectedListMeta", `${selectedQualified} qualified, ${selectedReviewNeeded} review needed, ${selectedRejected} rejected.`);
  ...
  setText("pipelineRejected", String(selectedRejected));
...
function renderListsPage() {
  ...
  setText("workspaceLastSync", appState.lastSyncAt ? `Last sync ${formatDate(appState.lastSyncAt)}` : "Waiting for first sync");
  if (selected) {
    const counts = listLeadCounts(selected.id);
    const qualified = counts.qualified || numberValue(selected.qualified);
    const reviewNeeded = counts.reviewNeeded || numberValue(selected.reviewNeeded);
    const rejected = counts.rejected || numberValue(selected.rejected);
