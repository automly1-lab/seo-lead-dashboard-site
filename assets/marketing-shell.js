(function(){
  ...
  var PUBLIC_RE=/\/(how-it-works|pricing|status|privacy|terms|refund-policy|login|signup|checkout-success|checkout-cancelled|checkout-pending)\/?$/;
  ...
  function page(){var p=location.pathname;if(/\/how-it-works\//.test(p))return'how';if(/\/pricing\//.test(p))return'pricing';if(/\/status\//.test(p))return'status';if(/\/privacy\//.test(p))return'privacy';if(/\/terms\//.test(p))return'terms';if(/\/refund-policy\//.test(p))return'refund';if(/\/login\//.test(p))return'login';if(/\/signup\//.test(p))return'signup';return'home';}
  ...
  function headerHtml(){
    ...
    return '<div class="rf-container rf-nav-inner"><a class="rf-brand" href="'+prefix()+'"><span class="rf-brand-mark">RF</span><span class="rf-brand-copy"><strong>RankForge</strong><small>by CrestlineOps</small></span></a><nav class="rf-nav-links" aria-label="Primary"><a '+(active==='home'?'class="active" aria-current="page"':'')+' href="'+prefix()+'">Home</a><a '+(active==='how'?'class="active" aria-current="page"':'')+' href="'+href('how-it-works/')+'">How it works</a><a '+(active==='pricing'?'class="active" aria-current="page"':'')+' href="'+href('pricing/')+'">Pricing</a><a '+(active==='status'?'class="active" aria-current="page"':'')+' href="'+href('status/')+'">Status</a></nav><div class="rf-nav-actions nav-actions">'+actions+'</div><button class="rf-menu-button" type="button" aria-label="Open menu" aria-expanded="false"><span></span></button></div><div class="rf-mobile-menu" hidden><a href="'+prefix()+'">Home</a><a href="'+href('how-it-works/')+'">How it works</a><a href="'+href('pricing/')+'">Pricing</a><a href="'+href('status/')+'">Status</a>'+(loggedIn()?'<a class="rf-mobile-cta" href="'+href('dashboard/')+'">Dashboard</a><a href="'+href('settings/')+'">Settings</a>'+(isAdmin()?'<a href="'+href('quality/')+'">Quality</a>':'')+'<button type="button" data-rf-signout>Sign out</button>':'<a href="'+href('login/')+'">Log in</a><a class="rf-mobile-cta" href="'+href('signup/')+'">Start free</a>')+'</div>';
  }
  ...
  function ensureFooter(){
    ...
    f.innerHTML='<div class="rf-container rf-public-footer-inner"><div><strong>RankForge by CrestlineOps</strong><p>Evidence-based SEO lead intelligence for agencies.</p></div><div class="rf-footer-cols"><div><h3>Product</h3><a href="'+prefix()+'">Home</a><a href="'+href('how-it-works/')+'">How it works</a><a href="'+href('pricing/')+'">Pricing</a><a href="'+href('status/')+'">Status</a><a href="'+href('dashboard/')+'">Dashboard</a></div><div><h3>Account</h3><a href="'+href('login/')+'">Log in</a><a href="'+href('signup/')+'">Start free</a></div><div><h3>Legal</h3><a href="'+href('privacy/')+'">Privacy</a><a href="'+href('terms/')+'">Terms</a><a href="'+href('refund-policy/')+'">Refund Policy</a></div></div></div>';
