(function(){
  ...
  window.rankforgeMvpCreditSystem={
    planInfo:function(options){
      if(window.rankforgeUserPlanResolver&&typeof window.rankforgeUserPlanResolver.getPlanInfo==='function'){
        return window.rankforgeUserPlanResolver.getPlanInfo(options);
      }
      var p=planInfo();
      var u=usage();
      return Promise.resolve({
        ...
        name:p.name||'Free',
        limit:p.searchLimit,
        leadLimit:p.creditLimit,
        maxLeadsPerBatch:p.maxBatch,
        label:p.searchLimit===Infinity?'Unlimited search batches':String(p.searchLimit)+' search batches/month',
        usage:{
          searchesThisMonth:u.searchUsed,
          qualifiedLeadsThisMonth:u.creditUsed,
          remainingSearches:p.searchLimit===Infinity?Infinity:Math.max(0,p.searchLimit-u.searchUsed),
          remainingQualifiedLeadCredits:p.creditLimit===Infinity?Infinity:Math.max(0,p.creditLimit-u.creditUsed)
        },
        profile:window.rankforgeUserProfile||null
      });
    }
  };
