(function(){
  ...
  function planMeta(plan,prof){
    ...
    if(plan==='stacked')return{name:'Stacked Paid Plan',copy:'This account has multiple active purchases stacked in the current billing period.',search:num(prof.monthly_search_limit,0),credits:num(prof.monthly_qualified_lead_credit_limit,0),batch:num(prof.max_leads_per_batch,50),csv:true,cta:'View pricing',href:'../pricing/#plans'};
  ...
  function effectivePlan(sess,prof){var admin=isAdmin(sess,prof);if(admin)return'admin';var status=billingStatus(prof&&prof.billing_status);if(status==='active')return normalizePlan(prof&&prof.plan);return 'free';}
  ...
  function setNavActive(){var links=Array.from(document.querySelectorAll('.rf-settings-section-nav a'));function update(){var current='account';links.forEach(function(a){var target=document.querySelector(a.getAttribute('href'));if(target&&target.getBoundingClientRect().top<180)current=(a.getAttribute('href')||'#account').replace('#','');});links.forEach(function(a){a.classList.toggle('active',(a.getAttribute('href')||'').replace('#','')===current);});}window.addEventListener('scroll',update,{passive:true});update();}
  ...
  function render(){
    var sess=session()||{};var prof=profile()||{};var admin=isAdmin(sess,prof);var billing=currentBilling(sess,prof);var plan=effectivePlan(sess,prof);var meta=planMeta(plan,prof);var used=usage(sess);var p=prefs();
    ...
    var pending=billing==='pending';var pendingBox=byId('pendingWarning');if(pendingBox)pendingBox.hidden=!pending;var cta=byId('planCta');if(cta){if(admin){cta.hidden=false;cta.textContent='Admin controls';cta.href='../quality/';}else{cta.hidden=false;cta.textContent=meta.cta;cta.href=meta.href;}}
    ...
    html('exportBadge',accessBadge(meta.csv));text('exportTitle',meta.csv?'CSV export enabled':'CSV export locked');text('exportCopy',meta.csv?'Export contact-ready prospects from your own workspace. Rejected leads remain hidden from normal users.':'CSV export is available on Starter, Growth, and Admin plans.');var exportCta=byId('exportCta');if(exportCta){exportCta.hidden=meta.csv;exportCta.href='../pricing/#plans';}
