(function(){
  ...
  function normalize(v){var raw=clean(v).toLowerCase().replace(/\s+/g,"_").replace(/-/g,"_");if(raw==="growth"||raw==="pro")return"growth";if(raw==="agency"||raw==="agency_intelligence")return"agency_intelligence";return"starter";}
  function getPlan(){try{return normalize(new URLSearchParams(location.search||"").get("plan")||"starter");}catch(e){return"starter";}}
  ...
  function init(){var form=document.getElementById("signupForm");if(!form||document.querySelector(".signup-selected-plan"))return;var key=getPlan();var plan=PLANS[key]||PLANS.starter;localStorage.setItem(PLAN_KEY,key);localStorage.setItem(BILLING_KEY,key==="agency_intelligence"?"waitlist":"pending_payment");var card=document.createElement("div");card.className="signup-selected-plan";card.innerHTML='<span class="workspace-label">Selected plan</span><strong>'+plan.name+'</strong><small>'+plan.price+' · '+plan.summary+'</small><p>This selection is saved as pending until checkout or manual account approval is connected.</p>';form.insertBefore(card,form.firstChild);var status=document.getElementById("authFormStatus");if(status)status.textContent=key==="agency_intelligence"?"Agency Intelligence is coming soon. Signup saves your interest and opens the workspace.":"After signup, your selected plan is saved as pending until checkout is connected.";}
