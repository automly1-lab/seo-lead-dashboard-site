...
(function () {
  ...
  const LEGACY_DASHBOARD_STORAGE_KEY = "rankforge-dashboard-state-v3";
  ...
  function clearAppWorkspaceCache() {
    localStorage.removeItem(APP_STORAGE_KEY);
    localStorage.removeItem(LEGACY_DASHBOARD_STORAGE_KEY);
  }

  function saveSession(session) {
    const previous = safeJson(localStorage.getItem(AUTH_SESSION_KEY), null);
    const previousUserId = previous && previous.userId ? String(previous.userId) : "";
    const nextUserId = session && session.userId ? String(session.userId) : "";

    if (!session) {
      localStorage.removeItem(AUTH_SESSION_KEY);
      localStorage.removeItem(CURRENT_USER_KEY);
      clearAppWorkspaceCache();
      return;
    }
