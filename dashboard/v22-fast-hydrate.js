(function(){
  ...
  function key(){var s=session()||{},x=st()||{},email=lower(s.email||(x.user&&x.user.email)),id=clean(s.userId||s.id||(x.user&&x.user.userId));return id||email||''}
  function latestStore(){
    var userKey=key();
    if(!userKey)return {searches:[],leads:[],audits:[]};
    var exact='rankforge-dashboard-data-v2::'+userKey;
    return safe(localStorage.getItem(exact),null)||{searches:[],leads:[],audits:[]};
  }
