(function(){
  ...
  function performDelete(index){
    ...
    window.requestAnimationFrame(refreshBatchUi);
  }
  ...
  function openDeleteModal(index){
    ...
    if(text) text.innerHTML='<strong>'+escapeHtml(titleOf(batch))+'</strong><br><span>This will remove the batch and its related leads from your dashboard view. Your completed reports and past activity will stay safe.</span>';
  ...
  function polishStatuses(){
    ...
    var idx=selectedIndex(); var selected=idx >= 0 && stateObj.searches ? stateObj.searches[idx] : null; var detailPill=document.querySelector('#searchBatchDetail .batch-status');
  ...
  document.addEventListener('click', function(ev){
    if(ev.target.closest && ev.target.closest('[data-view="searches"]')) window.requestAnimationFrame(refreshBatchUi);
    if(ev.target.closest && ev.target.closest('[data-batch-index]')) window.requestAnimationFrame(refreshBatchUi);
  }, true);

  function patchSearchesOnly(){
    if(window.__rf30SearchesOnlyPatched) return;
    if(typeof window.searches === 'function'){
      window.__rf30SearchesOnlyPatched=true;
      var prevSearches=window.searches;
      window.searches=function(){ prevSearches(); window.requestAnimationFrame(refreshBatchUi); };
    }
  }
  function boot(){
    injectCss();
    ...
    patchSearchesOnly();
    window.requestAnimationFrame(refreshBatchUi);
    setTimeout(function(){ patchSearchesOnly(); refreshBatchUi(); }, 250);
  }
